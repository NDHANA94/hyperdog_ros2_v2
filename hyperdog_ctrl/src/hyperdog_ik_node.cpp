...
private:
    ...
    void read_parameters()
    {
        ...
        // RCLCPP_INFO(this->get_logger(), "Loaded parameters: \n \t L1=%i \t L2=%i \t L3=%i", L1, L2, L3);
    }


    float* leg_ik(geometry_msgs::msg::Point p)
    {
        RCLCPP_INFO(this->get_logger(), "IK_func got the point: %f, %f, %f", p.x, p.y, p.z);
        // th1
        ...
        RCLCPP_INFO(this->get_logger(), "IK_func calculated angles: %f, %f, %f", joint_angles_[0], joint_angles_[1], joint_angles_[2]);
    ...
    void calculate_FR_joint_positions(geometry_msgs::msg::Point p){
        ...
        for(int i=0; i<3; i++){
            if(!std::isnan(jp_fr[i])){
                if(i==0){joint_positions_.fr[i] = -jp_fr[i];}
                else{joint_positions_.fr[i] = jp_fr[i];}
                is_valid_fr = true;
            }
            else{
                is_valid_fr = false;
    ...
    void calculate_BR_joint_positions(geometry_msgs::msg::Point p){
        ...
        for(int i=0; i<3; i++){
            if(!std::isnan(jp_br[i])){
                if(i==0){joint_positions_.br[i] = -jp_br[i];}
                else{joint_positions_.br[i] = jp_br[i];}
                is_valid_br = true;
            }
            else{
                is_valid_br = false;
    ...
    void calculate_joint_position(hyperdogv2_msgs::msg::FootPositions::SharedPtr msg){
        ...
        if(is_valid_fr){calculate_FL_joint_positions(msg->fl);}
        if(is_valid_fl){calculate_BR_joint_positions(msg->br);}
        if(is_valid_br){calculate_BL_joint_positions(msg->bl);}
        if(is_valid_bl){previous_joint_positions_ = joint_positions_;}
    }
    ...
    void publish_joint_angles(hyperdogv2_msgs::msg::FootPositions::SharedPtr msg)
    {
        ...
        RCLCPP_INFO(this->get_logger(), "FR joint angles: %f, %f, %f", joint_positions_.fr[0], joint_positions_.fr[1], joint_positions_.fr[2]);
        if(is_valid_fr && is_valid_fl && is_valid_br && is_valid_fl){
